...
To preview the styling without waiting for a compatible track, open **MetroLyrics** and press **Демо (Demo)**. The **Karaoke** section also has a live animated preview, three highlight modes (trail, active syllable, or active word), separate trail/active colors (each can follow the current track artwork accent), adjustable bold/pop effects, unsung-text opacity, and a global ±2000 ms timing correction.
